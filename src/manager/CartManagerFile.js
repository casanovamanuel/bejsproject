...
const FilePersistenceEngine = {
    ...
    find: async function (elementID) {//pasar a findByCode
        let contenido = await this.all()

        const res = contenido.find((elem) => elem.id === elementID)
        return res
    },
    findFirstBy: async function (filter) {
        let contenido = await this.all()

        const res = contenido.find((elem) => Object.keys(filter).every((elemKey) => elem[elemKey] == filter[elemKey])) //esto no se hace
        return res
    },
...
const CartManager = {
    ...
    getItem: async function (productId) { //esta para testear
        const filter = { productId }
        return await this.items.findFirstBy(filter)
    }
}
